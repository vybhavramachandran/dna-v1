...
class DnaEnv(gym.Env, utils.EzPickle):
    ...
    def createNucleus(self):
        ...
        self.nucleusTopBorderBody = self.world.CreateStaticBody(
            position=(self.FIRST_NUCLEOTIDE_X - 20, self.FIRST_NUCLEOTIDE_Y+20), angle=0, userData="NucleusTopBorder")
        self.nucleusRightBorderBody = self.world.CreateStaticBody(
            position=(self.FIRST_NUCLEOTIDE_X + 20, self.FIRST_NUCLEOTIDE_Y+20), angle=0, userData="NucleusRightBorder")
        self.nucleusLeftBorderBody = self.world.CreateStaticBody(
            position=(self.FIRST_NUCLEOTIDE_X - 20, self.FIRST_NUCLEOTIDE_Y+20), angle=0, userData="NucleusLeftBorder")
            ...
            if re.findall("^"+self.mRNA, self.templateDNAStrand) != []:
                if re.findall("^"+self.mRNA+"$", self.templateDNAStrand) != []:
                    ...
                    if distance<self.prev_distance:
                        self.reward = (100-distance)**2
                    else:
                        self.reward = -1 *((distance)**2)
